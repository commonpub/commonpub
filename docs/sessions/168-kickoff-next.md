...
**Remaining: migrate the live homepage `index.vue`** — now a code-dedup (the editor already matches the homepage's values via PageFrame, so this is NOT a WYSIWYG prerequisite). **CRITICAL ordering:** `.cpub-main-layout` is one scoped rule shared by ALL THREE homepage branches. Migrate **all three together** to `<PageFrame>`, keep the bespoke chrome (mobile-contest-hoist, `.cpub-sidebar-desktop/-mobile`, powered-badge) inside the relevant slots, THEN delete `.cpub-main-layout`/`.cpub-feed-col`/`.cpub-sidebar` grid CSS (keep bespoke-chrome CSS). Migrating one branch orphans the shared rule → live breakage. **Real-browser smoke** at ≥1025 / 768–1024 / ≤640px before merge: all 3 branches render the grid; mobile-hoist/sidebar/badge behave. (Optional: switch PageFrame's collapse to `@container` so the editor viewport-sim becomes accurate — verify on both the editor + public custom-page.)
