...
- ~~Editor canvas squish — palette/inspector collapse OR zoom controls~~ — **(A) DONE in session 161 round 4**. `useEditorChrome` composable owns two cookie-persisted booleans (`paletteHidden`, `inspectorHidden`). Toolbar gains two chevron toggle buttons flanking the viewport segmented control. Page grid dynamically reflows `grid-template-columns` based on visibility class (`--palette-hidden` / `--inspector-hidden`). `v-show` on the panels preserves component state across toggles. 9 tests cover the composable. **(B) zoom controls** still queued — would complement collapse for users who want full chrome + zoomed preview. Bigger lift (transform-origin math + scrollable canvas + zoom-to-cursor).
