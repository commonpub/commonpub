...
**infra**, so the publish set expands. Suggested order (each: bump, `pnpm test` green, publish, then
**poll `npm view` until the new version resolves before the next** — propagation lag, memory
...
3. **`@commonpub/infra`** 0.13 → 0.14 (email token interpolation + `copy?` template param).
4. **`@commonpub/auth`** 0.9 → 0.10 (RBAC signature changes from the security batch).
5. **`@commonpub/server`** 2.105 → 2.106 (privacy gates + contest email copy + registrations/reminders).
6. **`@commonpub/layer`** 0.97 → 0.98 via **`pnpm publish:layer`** (memory `feedback_pnpm_publish_layer`
   — never `npm publish` from layers/base; workspace:* literal).
