...
- **Inbox actor↔signer binding (session 187 audit, security).** `verifyInboxRequest` proves the SIGNER, but `processInboxActivity` trusts the JSON `body.actor` — different things. A validly-signed request from X could claim `actor: victim` (spoofed mirror requests/Accepts, mis-attributed federated content, like tampering). Fixed by `assertActorMatchesSigner(actorUri, body, label)` in `utils/inbox.ts`, called in all 3 inbox routes after verification (401 on host mismatch). ANY new inbox route must call it — the route is the only place this binding is enforced.

- **Registry (Phase 4, session 186): identity proven by ping signature; stats PULLED from NodeInfo, not trusted from the body.** `POST /api/registry/ping` (gated `actAsRegistry`) is verified by `verifyInboxRequest` so a domain can only register itself; the domain comes from the *verified* actor, and stats are pulled from the pinger's public NodeInfo (`fetchInstanceNodeInfo`, SSRF-guarded + same-host href). Auto-list on verified ping; admin `hidden`/`blocked` (re-ping preserves `hidden`, no-ops `blocked`). `announceToRegistry` is a SEPARATE default-OFF flag from `actAsRegistry` — federation never phones home unless opted in. Reuses Phase 3 for the per-entry Mirror/Request actions. Also: h3 types `Retry-After` as a NUMBER — `setResponseHeader(event,'Retry-After', n)` not `String(n)`, or the reference app's typecheck red-CIs (custom `X-*` headers accept strings).

- **"Push" mirroring = a consent-based REQUEST, not a mirror you operate (Phase 3, session 185).** `createMirror` is pull-only and **throws on `'push'`** — use `requestMirror()`, which writes a `mirror_requests` row (one unified table, `direction` incoming|outgoing) and sends an AP `Offer(Follow)` marked `cpub:mirrorRequest:true`. `instance_mirrors` stays pull-only. Inbound `Offer` → `onMirrorRequest` (only when cpub-marked, else non-CommonPub instances ignore it); approve → `Accept(Offer)` + a pull mirror of the requester (approver picks depth/filters), reject → `Reject(Offer)`; `onAccept`/`onReject` were *extended* (not new callbacks) to flip the requester's outgoing request by `offerActivityUri`. Wire-protocol change → bump protocol. See `codebase-analysis/09` for the full invariants.
...
## Contest editor & theming (session 211, `contests` branch — unreleased)

- **Every dark theme must set `color-scheme: dark`.** `:root` is `light`; dark themes override (`dark`/`agora-dark`/`stoa-dark` do). A dark theme without it renders native controls (date popups, scrollbars) light. stoa-dark fixed in 211; custom DB dark themes still don't set it.
- **`datetime-local` ↔ ISO: use `utils/datetime` (`toLocalInput`/`fromLocalInput`), never `toISOString().slice(0,16)`** — that's UTC, so the displayed time is offset-shifted. Prefer `CpubDateTimeField.vue`.
- **`layers/base/theme/` is a generated, gitignored copy** — edit `packages/ui/theme/` (the tracked source); run `node layers/base/scripts/bundle-theme.mjs` to refresh the dev copy.
- **Contest `format:'html'` is color-neutralized for dark-safety** — `sanitizeRichHtml(html,{neutralizeColors:true})` strips inline hardcoded colors (keeps `var()`/`currentColor`); the `.cpub-md-html` theme baseline shows through. Default OFF for general `sanitizeRichHtml`.

## Contest editor 3-panel shell (session 218, `contest-editor-shell` branch — unreleased)

- **Block-type keys MUST match between the palette/editor and `BlockContentRenderer`'s `componentMap`.** The palette inserts `horizontal_rule` but the renderer mapped only `divider`/`horizontalRule`, and its fallback only catches blocks with a `.html` field — so dividers rendered *nothing* in Preview + on the public page while looking fine in the write canvas. Fixed by aliasing `horizontal_rule`. When adding a palette block type, register the SAME key in `BlockContentRenderer` (and via `BLOCK_COMPONENTS_KEY` for the edit component) or it silently vanishes on render.
- **A block-editor shell must NOT be a `<form>`.** `EditorBlocks` palette items, `EditorSection` headers, and `BlockCanvas` controls are `<button>`s with no `type`, so inside a form they default to `type=submit` → every palette click / section toggle fires the form's save → refetch → re-hydrate wipes the dirty state. Use a `<div>` + explicit `@click` Save (matches `ProjectEditor`).
- **Dirty tracking on hoisted body editors:** watch `() => editor.blocks.value` (a getter), NOT the bare `readonly(blocks)` ref — the bare-ref deep watch fires on nested content edits but **misses structural inserts** (push/splice), so a content-less block (divider/image/judgesShowcase) wouldn't enable Save. The write-back also marks `formDirty` explicitly, guarded by a `syncingBodies` flag so hydrate/reseed doesn't false-dirty.
- **`@commonpub/editor/vue` components are package exports, not Nuxt auto-imports** — `EditorSection`/`EditorBlocks`/`BlockCanvas`/`useBlockEditor` must be explicitly `import`ed. A missing `EditorSection` import renders the rail flat (slot content leaks, no collapsible headers).
- **`html`-type blocks now render via the permissive `sanitizeRichHtml` everywhere** (was the stricter `sanitizeBlockHtml` fallback). Same trust tier as `CustomHtmlSection` (organizer/admin-authored); the raw content rides in Nuxt's escaped JSON hydration island (non-executable) and the client re-sanitizes before `v-html`.
