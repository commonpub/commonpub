 ...
 */
import { hexToHsl, hslToHex, rgba, adjL, clamp, readableOn } from './color.js';
...
export function buildPalette(opts: BuildPaletteOptions): Palette {
  ...
  const accentHover = dark ? adjL(accent, 8) : adjL(accent, -9);
  const accentDim = dark
    ? hslToHex(ah.h, Math.min(ah.s, 60), 18)
    : hslToHex(ah.h, Math.min(ah.s, 55), 90);
  const onAccent = readableOn(accent);
  const sat = clamp(ah.s, 45, 80);
  ...
  const rawHexes = [bg, surface, surface2, text, textSoft, textMuted, accent, sec];
