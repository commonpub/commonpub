...
| `actAsRegistry` | **OFF default** (Phase 4, session 186) | This instance acts as a registry: accepts signed `POST /api/registry/ping` heartbeats + serves `GET /api/registry/instances` + the admin Registry tab. Gates all `/api/registry/*` + `/api/admin/registry/*` routes. | admin |
| `announceToRegistry` | **OFF default** (Phase 4, session 186) | Send periodic signed heartbeats to `federation.registryUrl` (default `https://commonpub.io`) so this instance appears in that registry. SEPARATE from `actAsRegistry` so federation can be on without phoning home. Drives the `registry-heartbeat.ts` Nitro worker. | admin |
