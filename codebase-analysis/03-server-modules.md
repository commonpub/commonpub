...
- **Cohorts & advancement (Phase B2):** `advanceContestStage(db, contestId, userId, {reviewStageId, mode:'topN'|'manual', topN?, advancedEntryIds?})` — owner-gated; review-stage only; splits the surviving cohort (entries not already `eliminated`) into advancers + eliminated, snapshots each entry's round score/rank into `contest_entries.stage_state`, and moves `currentStageId` to the next stage. **Idempotent per stage** (replaces that stage's `stage_state` rows). `topN` ties broken score→rank→id. `isEliminated(entry)` = any `stage_state` row with status `eliminated`. `calculateContestRanks` **excludes eliminated entries** (jsonb `NOT (stage_state @> '[{"status":"eliminated"}]')`); `listContestEntries` surfaces `stageState` + a derived `eliminated`. Route: `POST /api/contests/[slug]/advance` (`contestAdvanceSchema`). **`judgeContestEntry` is cohort-gated** (session 189) — it rejects eliminated entries, so later judging rounds only score survivors. **Per-round rubric:** each `review` stage may carry its own `criteria` (on `ContestStage`); the judge page uses the current review stage's criteria, falling back to the contest-level `judgingCriteria`. **`ContestStage.advanceCount`** defines the Top-N cut ("winners") for a review round (set in the editor; the Advancement control pre-fills it). `advanceContestStage` still takes an explicit `topN`/`advancedEntryIds` — `advanceCount` is the saved default the UI passes. **Voting** (`communityVotingEnabled`) is advisory only — never affects ranks or advancement; judge `score` drives both. **Per-round score isolation** (session 189): each `JudgeScoreEntry` carries a `roundId`; a judge has one score per round and the entry's live `score` aggregates only the current round, so later rounds don't overwrite earlier ones (earlier rounds stay in `judgeScores` as history). `ContestStage.advanceCount` defines the Top-N cut.
