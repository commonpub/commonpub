...
| 0022 | `wide_sumo` | contest `content_format` column + `contest_content_format` enum (markdown\|html). Session 197. **Column DROPPED in 0031** (superseded by the per-field `*_format` columns in 0023); the `contest_content_format` enum survives (used by those columns). |
...
| 0026 | `remote_like_count` | `content_items.remote_like_count` + `hub_posts.remote_like_count` integer (default 0, NOT NULL) — federated like aggregation kept separate from local likes. |
| 0027 | `audit204_indexes_dedup` | `processed_activities` + `digest_runs` tables + composite indexes (`content_items(author_id,status)`, `(type,status,published_at)`, `notifications(user_id,created_at,id)`). Session 204 audit. |
| 0028 | `worthless_silver_centurion` | `contests.description_blocks` + `contests.rules_blocks` jsonb (BlockTuple[]; legacy text kept, converted on edit). Contest Phase 2 block-editor body. Branch `contests`. |
...
| contestJudges | Judge roster (THE source of truth) | unique(contestId, userId); role enum (lead/judge/guest); invitedAt/acceptedAt — scoring needs accepted + non-guest; can't judge own entry. The route `/contests/:slug/judge` now asserts the entry belongs to that contest (B5a, Phase 6) |
...
| contestEntryPrivateFields | **PII partition (0030, Phase 4)** | `unique(entryId)`; `fields` jsonb holds address/email/etc. NEVER returned by the `/entries` endpoints — only the gated `/entries/:entryId/private` route + the export's PII columns (both `contest.pii`-gated, entrant sees own). `validateSubmissionFields` splits PII out of the public `stageSubmissions` |
| contestAgreementAcceptances | **Versioned consent (0030, Phase 4)** | append-only per acceptance: `termsHash` + `termsSnapshot` + `ip` + `acceptedAt`, captured atomically with the submission. Audit/legal record |
