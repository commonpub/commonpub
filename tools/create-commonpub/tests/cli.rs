...
fn package_json_pins_current_commonpub_versions() {
    ...
    assert!(pkg.contains("\"@commonpub/layer\": \"^0.64.0\""), "layer pin must be ^0.64.0");
    assert!(pkg.contains("\"@commonpub/server\": \"^2.82.0\""), "server pin must be ^2.82.0");
    assert!(pkg.contains("\"@commonpub/schema\": \"^0.35.0\""), "schema pin must be ^0.35.0");
    assert!(pkg.contains("\"@commonpub/config\": \"^0.19.0\""), "config pin must be ^0.19.0");
