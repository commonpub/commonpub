...
fn package_json_pins_current_commonpub_versions() {
    ...
    assert!(pkg.contains("\"@commonpub/layer\": \"^0.49.0\""), "layer pin must be ^0.49.0");
    assert!(pkg.contains("\"@commonpub/server\": \"^2.74.0\""), "server pin must be ^2.74.0");
    assert!(pkg.contains("\"@commonpub/schema\": \"^0.27.0\""), "schema pin must be ^0.27.0");
