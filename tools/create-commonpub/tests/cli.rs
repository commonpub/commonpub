...
fn package_json_pins_current_commonpub_versions() {
    ...
    assert!(pkg.contains("\"@commonpub/layer\": \"^0.106.0\""), "layer pin must be ^0.106.0");
    assert!(pkg.contains("\"@commonpub/server\": \"^2.113.0\""), "server pin must be ^2.113.0");
    assert!(pkg.contains("\"@commonpub/schema\": \"^0.59.0\""), "schema pin must be ^0.59.0");
    assert!(pkg.contains("\"@commonpub/config\": \"^0.33.0\""), "config pin must be ^0.33.0");
