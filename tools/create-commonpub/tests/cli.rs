...
fn package_json_pins_current_commonpub_versions() {
    ...
    assert!(pkg.contains("\"@commonpub/layer\": \"^0.85.0\""), "layer pin must be ^0.85.0");
    assert!(pkg.contains("\"@commonpub/server\": \"^2.91.0\""), "server pin must be ^2.91.0");
    assert!(pkg.contains("\"@commonpub/schema\": \"^0.47.0\""), "schema pin must be ^0.47.0");
    assert!(pkg.contains("\"@commonpub/config\": \"^0.23.0\""), "config pin must be ^0.23.0");
