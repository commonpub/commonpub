...
fn package_json_pins_current_commonpub_versions() {
    ...
    assert!(pkg.contains("\"@commonpub/layer\": \"^0.86.7\""), "layer pin must be ^0.86.7");
    assert!(pkg.contains("\"@commonpub/server\": \"^2.94.0\""), "server pin must be ^2.94.0");
    assert!(pkg.contains("\"@commonpub/schema\": \"^0.49.0\""), "schema pin must be ^0.49.0");
