...
fn package_json_pins_current_commonpub_versions() {
    ...
    assert!(pkg.contains("\"@commonpub/layer\": \"^0.28.0\""), "layer pin must be ^0.28.0");
    assert!(pkg.contains("\"@commonpub/server\": \"^2.62.0\""), "server pin must be ^2.62.0");
    assert!(pkg.contains("\"@commonpub/schema\": \"^0.21.0\""), "schema pin must be ^0.21.0");
