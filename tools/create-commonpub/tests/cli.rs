...
fn package_json_pins_current_commonpub_versions() {
    ...
    assert!(pkg.contains("\"@commonpub/layer\": \"^0.30.0\""), "layer pin must be ^0.30.0");
    assert!(pkg.contains("\"@commonpub/server\": \"^2.64.0\""), "server pin must be ^2.64.0");
    assert!(pkg.contains("\"@commonpub/schema\": \"^0.23.0\""), "schema pin must be ^0.23.0");
