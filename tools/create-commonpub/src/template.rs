...
pub fn render_package_json(config: &InstanceConfig) -> String {
    let mut deps = vec![
        r#"    "@commonpub/config": "^0.3.1""#.to_string(),
        r#"    "@commonpub/schema": "^0.3.1""#.to_string(),
        r#"    "@commonpub/auth": "^0.3.1""#.to_string(),
        r#"    "@commonpub/ui": "^0.3.1""#.to_string(),
        r#"    "@commonpub/server": "^0.3.1""#.to_string(),
        r#"    "@commonpub/infra": "^0.3.1""#.to_string(),
    ];

    if config.feature_content {
        deps.push(r#"    "@commonpub/editor": "^0.3.1""#.to_string());
    }
    if config.feature_docs {
        deps.push(r#"    "@commonpub/docs": "^0.3.1""#.to_string());
    }
    if config.feature_learning {
        deps.push(r#"    "@commonpub/learning": "^0.3.1""#.to_string());
    }
    if config.feature_explainers {
        deps.push(r#"    "@commonpub/explainer": "^0.3.1""#.to_string());
    }
    if config.feature_federation {
        deps.push(r#"    "@commonpub/protocol": "^0.3.1""#.to_string());
    }
