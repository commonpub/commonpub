...
export const load: PageServerLoad = async ({ locals, url }) => {
  ...
  const [featured, listing] = await Promise.all([
    page === 1 ? listContent(locals.db, { status: 'published', type: 'article', featured: true, limit: 1 }) : Promise.resolve({ items: [], total: 0 }),
    listContent(locals.db, { status: 'published', type: 'article', sort, limit, offset }),
  ]);
