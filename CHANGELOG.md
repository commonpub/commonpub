...
`?start=N` integer-seconds parameter the embed iframe accepts. Adds
Vimeo private-video hash support — `vimeo.com/ID/HASH` rewrites to
...
+6to4/NAT64). New `contentImport` flag (default on) gates
`importFromUrl` (rule #2). Unsigned remote-GET federation fetches
migrated to `safeFetch`. Signed-request + inbound-digest/sig
migration deferred to Stage 3 (needs the two-instance interop test).
All dormant in prod (`features.federation` off).
